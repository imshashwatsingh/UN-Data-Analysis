...
def num_countries(df):
    ...
    # print(df.columns)
    val_cou = df['Region_combined'].value_counts()
    val_cou=val_cou.reset_index().rename({'index' : "Region_combined" , 'count':'Number of countries'} , axis=1)
    
    fig = px.pie(val_cou, values='Number of countries', names= "Region_combined" , title='Number of countries in each region')
    return fig
...
def pop_reg(df):
    pop = df[['country' , 'Region_combined' , 'Population (2017)']].groupby('Region_combined').mean("Population (2017)").sort_values(by='Population (2017)' , ascending=False)
    pop = pop.reset_index()
